...
DB_FILE = os.path.join(BASE_DIR, "public_survey_data.csv")
CRED_FILE = os.path.join(BASE_DIR, "merchant_accounts.csv")
        ...
        if not df_sug.empty:
            ...
            with col_kw:
                ...
                if top_words:
                    ...
                    # --- 改用 Plotly 绘制交互式条形图 ---
                    df_kw = pd.DataFrame({'关键词': list(words), '提及次数': list(counts)})
                    # Plotly 的横向柱状图默认是从下往上排，所以按升序排列让最多的词在最上面
                    df_kw = df_kw.sort_values(by='提及次数', ascending=True)
                    ...
                    fig_kw = px.bar(
                        ...
                        y='关键词', 
                        orientation='h',
                        title=t("高频词 Top 8", "Top 8 Keywords"),
                        color='提及次数',
                        color_continuous_scale='Blues'
                    )
                    
                    # 设置鼠标悬停提示 (Hover) 和图表布局
                    fig_kw.update_traces(hovertemplate="<b>%{y}</b><br>提及次数: %{x} 次<extra></extra>")
                    fig_kw.update_layout(
                        xaxis_title=t("提及次数", "Mentions"),
                        yaxis_title="",
                        margin=dict(l=0, r=0, t=40, b=0),
                        coloraxis_showscale=False # 隐藏右侧的颜色条，让图表更宽敞
                    )
                    fig_kw.update_xaxes(tickformat="d") # 强制 X 轴只显示整数
        ...
        if st.button(t("生成对比图", "Generate Comparison Plot")):
            if col_item and col_sales:
                ...
                if df_actual.empty: 
                    ...
                    # --- 改用 Plotly 绘制交互式分组柱状图 ---
                    fig2 = px.bar(
                        comp_df,
                        barmode='group', # 设置为分组并排显示
                        title=t("实际销售 和 公众期望 差异对比", "Actual Sales vs Public Demand"),
                        color_discrete_sequence=['#1f77b4', '#ff7f0e']
                    )
                    
                    # 优化图表布局和鼠标悬停提示
                    fig2.update_layout(
                        xaxis_title=t("商品名称", "Item Name"),
                        yaxis_title=t("百分比 (%)", "Percentage (%)"),
                        legend_title=t("指标", "Metrics"),
                        margin=dict(t=40, b=0)
                    )
                    fig2.update_traces(hovertemplate="<b>%{x}</b><br>%{data.name}: %{y:.1f}%<extra></extra>")
